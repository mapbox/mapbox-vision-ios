...
final class RecordCoordinator {
    ...
    private var currentRecordingPath: RecordingDir?
    ...
    init() {
        self.videoRecorder = VideoBuffer(chunkLength: VideoBufferConstant.defaultChunkLength,
                                         chunkLimit: VideoBufferConstant.defaultChunkLimit)
        videoRecorder.delegate = self
        
        createFolder(path: LocationInDocumentDir.recordingsDir.path)
    }
    ...
    func startRecording(referenceTime: Float, directory: String? = nil, videoSettings: VideoSettings) throws {
        ...
        let cachePath = LocationInDocumentDir.cacheDir.path
        recreateFolder(path: LocationInDocumentDir.currentRecordingDir.path)
        recreateFolder(path: cachePath)

        if let directory = directory {
            currentRecordingPath = RecordingDir(directoryFullPath: directory, fileExtension: videoSettings.fileExtension)
        } else {
            currentRecordingPath = RecordingDir(basePathInDocumentDir: .currentRecordingDir, fileExtension: videoSettings.fileExtension)
        }
        
        jsonWriter = FileRecorder(path: currentRecordingPath!.videosLogPath)
        
        currentStartTime = DispatchTime.now()
        currentEndTime = nil
        
        videoRecorder.chunkLength = savesSourceVideo ? 0 : VideoBufferConstant.defaultChunkLength
        videoRecorder.chunkLimit = savesSourceVideo ? 1 : VideoBufferConstant.defaultChunkLimit
        videoRecorder.startRecording(to: cachePath, settings: videoSettings)
        
        delegate?.recordingStarted(path: currentRecordingPath!.fullPath)
    }
    ...
    func clearCache() {
        RecordingDir.clear(basePath: .recordingsDir)
    }
    ...
    private func recordingStopped() {
        ...
        if let path = currentRecordingPath {
            do {
                if abortRecording {
                    try FileManager.default.removeDir(at: path.fullPath)
                } else if path.basePath != .custom {
                    try path.move(to: .recordingsDir)
//                    try FileManager.default.move(recordingDir: , to: .recordingsDir)
                }
            } catch {
    ...
    private func chunkPath(for number: Int, fileExtension: String) -> String {
        return "\(LocationInDocumentDir.cacheDir.path)/\(number).\(fileExtension)"
    }
